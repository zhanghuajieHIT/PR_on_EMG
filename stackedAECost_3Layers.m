function [ cost, grad ] = stackedAECost_3Layers(theta, inputSize, hiddenSize, ...
...
Z4 = stack{3}.w*A3 + repmat(stack{3}.b,1,M);    % hiddenSizeL3×M
A4 = sigmoid(Z4);                               % output of hidden layer 3
% output of the whole network
A5 = softmaxProbability( softmaxTheta, [A4;ones(1,M)] );    % numClasses×M

cost = (-1/M)*sum(sum(log(A5).*groundTruth)) + (lambda/2)*sum(sum(softmaxTheta.^2));

% 使用反向传播计算每层的残差
Delta5 = A5-groundTruth;                                   % numClasses×M
Delta4 = (softmaxTheta(:,1:end-1)'*Delta5).*A4.*(1-A4);    % hiddenSizeL3×M
Delta3 = ((stack{3}.w)'*Delta4).*A3.*(1-A3);               % hiddenSizeL2×M
...
softmaxThetaGrad = (-1/M)*[A4;ones(1,M)]*(groundTruth-A5)'+lambda*softmaxTheta';
